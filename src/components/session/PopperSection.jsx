      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2.5">
        {playerIds.map(id => (
          <div key={id} className="flex items-center gap-2">
            ...
            <span className="flex-1 text-xs text-gray-600 min-w-0">{playerName(id)}</span>
            <div className="flex items-center gap-1.5 shrink-0">
              <button
                type="button"
                onClick={() => adjust(id, -1)}
                className="w-9 h-9 flex items-center justify-center rounded-lg border border-gray-200 text-gray-500 hover:bg-gray-50 text-lg leading-none"
              >
                −
              </button>
